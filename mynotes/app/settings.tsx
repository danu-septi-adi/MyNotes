...
export default function SettingsScreen() {
  const { settings, setCurrency, setTheme, setTabColor } = useSettings();
  ...
  return (
    <View style={[st.c, { backgroundColor: c.bg }]}>
      ...
      <ScrollView contentContainerStyle={st.content}>
        {/* Warna Tema */}
        <Text style={[st.sectionTitle, { color: c.textSecondary }]}>Warna Tema</Text>
        <View style={[st.card, { backgroundColor: c.surface }]}>
          <TouchableOpacity style={st.colorRow} onPress={() => { setTempColor(settings.tabColor); setColorModal(true); }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[st.rowLabel, { color: c.text }]}>Warna Aksen</Text>
              <Text style={[st.rowSub, { color: c.textMuted }]}>Saat ini: {settings.tabColor}</Text>
            </View>
        ...
        {/* Mata Uang */}
        <Text style={[st.sectionTitle, { color: c.textSecondary }]}>Mata Uang</Text>
        <View style={[st.card, { backgroundColor: c.surface }]}>
          {currencies.map((cur, i) => (
            <TouchableOpacity
              ...
              {settings.currency === cur.code && <MaterialCommunityIcons name="check-circle" size={22} color={c.primary} />}
            </TouchableOpacity>
          ))}
        </View>

        {/* Tema */}
        <Text style={[st.sectionTitle, { color: c.textSecondary }]}>Mode Tampilan</Text>
        <View style={[st.card, { backgroundColor: c.surface }]}>
          ...
          <View style={[st.modalContent, { backgroundColor: c.surface }]}>
            <View style={st.modalHeader}>
              <TouchableOpacity onPress={() => setColorModal(false)}>
                <Text style={[st.cancelText, { color: c.textMuted }]}>Batal</Text>
              </TouchableOpacity>
              <Text style={[st.modalTitle, { color: c.text }]}>Pilih Warna</Text>
              <TouchableOpacity onPress={() => { setTabColor(tempColor); setColorModal(false); }}>
                <Text style={[st.saveText, { color: c.primary }]}>Simpan</Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={st.presetsRow}>
              {['#6366F1','#8B5CF6','#3B82F6','#10B981','#14B8A6','#F59E0B','#F97316','#EF4444','#EC4899','#6B7280'].map(clr => (
                <TouchableOpacity key={clr} style={[st.presetDot, { backgroundColor: clr }, tempColor === clr && { borderWidth: 3, borderColor: '#fff' }]}
                  onPress={() => setTempColor(clr)} />
              ))}
            </View>
          </View>
        </View>
...
const st = StyleSheet.create({
  ...
  sectionTitle: { ...Typography.smallBold, marginBottom: Spacing.sm, marginTop: Spacing.xl, paddingHorizontal: Spacing.xs },
